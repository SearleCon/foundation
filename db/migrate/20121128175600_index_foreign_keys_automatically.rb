class IndexForeignKeysAutomatically < ActiveRecord::Migration
  def up
  ...
  def down
    ...
    tables.each do |table|
      ...
      foreign_keys = columns.find_all{|column| column.name.include? '_id'}
      foreign_keys.each do |foreign_key|
        remove_index table, foreign_key.name if ActiveRecord::Base.connection.index_exists?(table, foreign_key.name)
      end
    end
  end
end
